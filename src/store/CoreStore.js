...
import { isArray, isEmpty, defer, isFunction, upperFirst } from 'lodash';
    ...
    */
    buildEachNodeChangeEventListener() {
        ...
        for (let definition in this.definition) {
            const capitalizeDefinition = upperFirst(definition);
            ...
            for (let node in rawData) {
                if (currentStore.definition[node]) {
                    ...
                    if (currentStore.customHandler && currentStore.customHandler[node] && currentStore.customHandler[node][type]) {
                        ...
                        if (!isFunction(currentStore[`${type}${upperFirst(node)}`])) {
                            throw new Error(`The listener you try to call is unavailable : ${type}${upperFirst(node)}`);
                        }
                        currentStore[`${type}${upperFirst(node)}`](rawData[node], status[node], otherInformations);
                    }
                }
            }
