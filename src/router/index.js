...
const createRouter = Backbone => Backbone.Router.extend({
    ...
    route: function (urlRoute, name, callback) {
        ...
        function customWrapperAroundCallback() {
            let currentRoute = urlRoute;
            //Rebuild the callback arguments.
            let routeArguments = [urlRoute, ...arguments];
            ...
            let routeName = '';//siteDescriptionBuilder.findRouteName(currentRoute);
            let routeDescciption = { roles: ['DEFAULT_ROLE'] };//siteDescriptionBuilder.getRoute(routeName);
