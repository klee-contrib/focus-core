  ...
  propertyValue =  ()=>
    ...
    if this[property]?
      ...
      if dataType is "date" and this[property] isnt ""
        return "<div class='#{metadataClass}'>#{propValue}</div>"
    ...
    <div class='#{radioSize}'>
         <div class='input-group'>
             <span class='input-group-addon'>
                <input id='#{property + value}' name='#{property}' data-name='#{property}' type=radio value='#{value}' #{checkedByDefault() if isDefault} #{inputAttributes} #{placeholder} #{readonly} #{disabled}/>      
             </span>
...
Handlebars.registerHelper "options_selected", (property, options) ->
  ...
  addOption = (elt) ->
    ...
    return undefined
  multiple = if opt.isMultiple then "multiple style='width:100%;'" else ""
  #Initialize the html  
  html = "<div class='form-group #{error}'>
            ...
            <div class='controls #{inputSize}'>
              <div class='input-group'>
                #{icon()}                
                <select id='#{property}' data-name='#{property}' #{multiple} #{readonly} #{optName} #{optToTriggerName} #{optToTriggerListKey} #{dataMapping} class='form-control input-sm'>"
